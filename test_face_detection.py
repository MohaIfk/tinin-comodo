...
def test_face_detection():
    """
    Test the improved face detection implementation.
    
    This script tests the changes made to address the following issues:
    1. False positives in human detection
    2. Multiple faces detected on a single person
    3. Faces detected outside the bounding box of the person
    ...
    """
    logger.info("=== Face Detection Test ===")
    ...
    try:
        while True:
            ...
            cv2.putText(
                annotated_frame,
                f"FPS: {fps:.1f} | Humans: {human_count}",
                (10, 30),
        ...
        logger.error(f"Error during face detection test: {e}")
    finally:
        # Release resources
        cap.release()
        cv2.destroyAllWindows()
        logger.info("=== Face Detection Test Complete ===")
